...
sender_email = os.getenv("jamsheerkhan118@gmail.com")
sender_password = os.getenv("snby caik mngj kalx")
receiver_email = ["jamsheerkhan117@gmail.com"]
...
def process_video(video_path, output_path):
    ...
    while cap.isOpened():
        ...
        if detected_now and not screenshot_taken:
            ...
            # Get camera location from DB
            db_loc = get_db()
            cursor_loc = db_loc.cursor(dictionary=True)
            cursor_loc.execute("SELECT location FROM cameras WHERE id=%s", (camera_id,))
            cam = cursor_loc.fetchone()
            cursor_loc.close()
            db_loc.close()
            location = cam["location"] if cam and cam["location"] else get_location()
